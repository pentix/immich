...
export class SmartInfoRepository implements ISmartInfoRepository {
  ...
  async searchByEmbedding({ ownerId, embedding, minDistance }: EmbeddingSearch): Promise<AssetEntity[]> {
    const results = await this.repository
      .createQueryBuilder('smartInfo')
      .leftJoinAndSelect('smartInfo.asset', 'asset')
      .where('asset.ownerId = :ownerId', { ownerId })
      .andWhere(`(smartInfo.clipEmbedding <=> ${asVector(embedding)}) <= :minDistance`, { minDistance })
  ...
  async upsert(info: Partial<SmartInfoEntity>): Promise<void> {
    ...
    if (clipEmbedding) {
      await this.repository.manager.query(
        `UPDATE "smart_info" SET "clipEmbedding" = '[${clipEmbedding.join(',')}]' WHERE "assetId" = $1`,
        [info.assetId],
      );
    }
  }
